...
function OddButton({ data }) {
  ...
  const currentOption = appContext.selectedEvents[0] && appContext.selectedEvents[0].optionId;
  const currentOptionIsSelected = currentOption == `${data.eventId}-${data.title}`;
  if (!currentOptionIsSelected && selected) {
    setSelected(false);
  }
  return (
    <button className={`${styles.oddButton} ${selected ? styles.selected : ''}`} onClick={() => {
      if (selected) {
        setAppContext({
          ...appContext,
          selectedEvents: []
